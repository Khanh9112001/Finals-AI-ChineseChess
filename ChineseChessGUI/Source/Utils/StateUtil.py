...
class StateUtil:
    ...
    def kingGo_Recommends(state, i, j, lowercaseMove=True):
        ...
        if lowercaseMove:
            # blackside
            if (state[i][j] == "k"):
                if (i == 0 and state[i + 1][j].islower() == False and StateUtil.checkKingOpposite(state, i + 1, j, True) == False):
                    sts[i + 1][j] = 'o'
                elif (i == 1):
                    if state[i - 1][j].islower() == False:
                        sts[i - 1][j] = 'o'
                    if state[i + 1][j].islower() == False and StateUtil.checkKingOpposite(state, i + 1, j, True) == False:
                        sts[i + 1][j] = 'o'
                elif (i == 2 and state[i - 1][j].islower() == False):
                    sts[i - 1][j] = 'o'
                if (j == 3 and state[i][j + 1].islower() == False and StateUtil.checkKingOpposite(state, i, j + 1, True) == False):
                    sts[i][j + 1] = 'o'
                elif (j == 4):
                    if state[i][j - 1].islower() == False and StateUtil.checkKingOpposite(state, i, j - 1, True) == False:
                        sts[i][j - 1] = 'o'
                    if state[i][j + 1].islower() == False and StateUtil.checkKingOpposite(state, i, j + 1, True) == False:
                        sts[i][j + 1] = 'o'
                elif (j == 5 and state[i][j - 1].islower() == False and StateUtil.checkKingOpposite(state, i, j - 1, True) == False):
                    sts[i][j - 1] = 'o'
        # red
        else:
            # redside
            if (state[i][j] == "K"):
                if (i == 7 and state[i + 1][j].isupper() == False):
                    ...
                    if state[i - 1][j].isupper() == False and StateUtil.checkKingOpposite(state, i - 1, j, False) == False:
                        sts[i - 1][j] = 'o'
                    if state[i + 1][j].isupper() == False:
                        sts[i + 1][j] = 'o'
                elif (i == 9 and state[i - 1][j].isupper() == False and StateUtil.checkKingOpposite(state, i - 1, j, False) == False):
                    sts[i - 1][j] = 'o'
                if (j == 3 and state[i][j + 1].isupper() == False and StateUtil.checkKingOpposite(state, i, j + 1, False) == False):
                    sts[i][j + 1] = 'o'
                elif (j == 4):
                    if state[i][j - 1].isupper() == False and StateUtil.checkKingOpposite(state, i, j - 1, False) == False:
                        sts[i][j - 1] = 'o'
                    if state[i][j + 1].isupper() == False and StateUtil.checkKingOpposite(state, i, j + 1, False) == False:
                        sts[i][j + 1] = 'o'
                elif (j == 5 and state[i][j - 1].isupper() == False  and StateUtil.checkKingOpposite(state, i, j - 1, False) == False):
                    sts[i][j - 1] = 'o'
    ...
    @staticmethod
    def change_upper_to_lower(state):
        for i in range(0,10):
            for j in range(0,9):
